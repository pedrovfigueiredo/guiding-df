...
public:
	...
	*/
	...
	static void printToConsole(const Profiler::Capture::SharedPtr& capture) {
        if (!capture) {
            std::cout << "No capture data available." << std::endl;
            return;
        }

        std::cout << "===== Profiler Results =====" << std::endl;
        std::cout << "Frame count: " << capture->getFrameCount() << std::endl;
        
        const auto& lanes = capture->getLanes();
        for (const auto& lane : lanes) {
            std::cout << "\nEvent: " << lane.name << std::endl;
            std::cout << "  Min: " << std::fixed << std::setprecision(3) << lane.stats.min << " ms" << std::endl;
            std::cout << "  Max: " << std::fixed << std::setprecision(3) << lane.stats.max << " ms" << std::endl;
            std::cout << "  Mean: " << std::fixed << std::setprecision(3) << lane.stats.mean << " ms" << std::endl;
            std::cout << "  StdDev: " << std::fixed << std::setprecision(3) << lane.stats.stdDev << " ms" << std::endl;
        }
        std::cout << "=========================" << std::endl;
    }

    static void saveToCSV(const Profiler::Capture::SharedPtr& capture, const std::string& filename) {
        if (!capture) {
            return;
        }
        ...
        // Write header
        file << "Event,Min (ms),Max (ms),Mean (ms),StdDev (ms)";
        
        // Add frame numbers if there are records
        const auto& lanes = capture->getLanes();
        if (!lanes.empty() && !lanes[0].records.empty()) {
            for (size_t i = 0; i < lanes[0].records.size(); ++i) {
                file << ",Frame " << i;
            }
        }
        file << "\n";
        ...
        for (const auto& lane : lanes) {
            ...
            // Write per-frame data if available
