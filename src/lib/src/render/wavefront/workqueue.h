...
public:
    ...
    KRR_CALLABLE int pushCameraRay(Ray ray, uint pixelId, const Color& startingThp = Color::Ones()) {
        int index = allocateEntry();
        this->depth[index] = 0;
		this->thp[index]	 = startingThp;
...
class SamplesCountQueue : public WorkQueue<SamplesCountWorkItem> {
public:
    using WorkQueue::WorkQueue;
    using WorkQueue::push;
};
