...
float calc_metric_relmse_epsilon(const Color4f *frame, const Color4f *reference, 
	size_t n_elements) {
	Color3f *error_buffer = initialize_metric(n_elements);
	...
	GPUParallelFor(n_elements, [=] KRR_DEVICE(int pixel) mutable {
		...
		for (int ch = 0; ch < Color::dim; ch++) {
			error_buffer[pixel][ch] =
				reference[pixel][ch] == difference[ch] / (reference[pixel][ch] + EPSILON);	
		}
	});
	return thrust::transform_reduce(
			   thrust::device, error_buffer, error_buffer + n_elements,
...
KRR_CALLABLE Color rel_mse_epsilon(const Color &y, const Color &ref) {
	Color ret{}, diff = (y - ref).abs();
	const float EPSILON = 1e-2f;
	for (int ch = 0; ch < Color::dim; ch++) {
		ret[ch] = pow2(diff[ch] / (ref[ch] + EPSILON));
	}
	return ret;
}
		...
		[=] KRR_DEVICE(Color y, Color ref) -> Color {
			...
			switch (metric) {
				...
				case ErrorMetric::SMAPE:
					return smape(y, ref);
				case ErrorMetric::RelMSEEpsilon:
					return rel_mse_epsilon(y, ref);
