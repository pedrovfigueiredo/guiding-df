...
public:
	...
	KRR_CALLABLE T min(const T& val) {
#ifdef KRR_DEVICE_CODE
		return atomicMin((T*) & m_val, val);
#else
		auto cur = m_val.load();
		/* keep trying adding that atomic float, until no one has changed it in between. */
		while (cur > val && !m_val.compare_exchange_weak(cur, val));
		return cur;
#endif
	}

	KRR_CALLABLE T max(const T& val) {
#ifdef KRR_DEVICE_CODE
		return atomicMax((T*) & m_val, val);
#else
		auto cur = m_val.load();
		/* keep trying adding that atomic float, until no one has changed it in between. */
		while (cur < val && !m_val.compare_exchange_weak(cur, val));
