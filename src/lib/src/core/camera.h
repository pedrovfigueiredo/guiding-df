...
public:
	...
	KRR_CALLABLE Ray getRay(Vector2i pixel, Vector2i frameSize, Sampler& sampler, bool fixedSample = false) {
		Ray ray;
		/* 1. Statified sample on the film plane (within the fragment) */
		Vector2f sampledNoise = fixedSample ? Vector2f(0.0f) : sampler.get2D();
		Vector2f p = (Vector2f)pixel + Vector2f(0.5f) + sampledNoise; // uniform sample + box filter
