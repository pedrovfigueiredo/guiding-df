...
void PPGPathTracer::setScene(Scene::SharedPtr scene) {
	scene->mTwoSidedLights = false; // Edit this setting to consider lights as twosided
	scene->toDevice();
	mpScene = scene;
...
void PPGPathTracer::initialize(){
	...
	// cudaDeviceSetLimit(cudaLimitStackSize, 65536ULL);
	Allocator& alloc = *gpContext->alloc;
	...
	// We can reasonably assume that having 10K different possible launch seeds is enough; and that each launch will happen within 1sec of each other (so time(NULL) changes).
