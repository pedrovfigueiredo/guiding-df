...
template <typename T> KRR_CALLABLE T safe_acos (T x)
{
  if (x < -1.0) x = -1.0;
  else if (x > 1.0) x = 1.0;
  return acos(x);
...
// Square and Uniform Sphere
KRR_CALLABLE Vector3f squareToUniformSphere(Vector2f u) {
	float z = 1 - 2 * u[0];
...
KRR_CALLABLE Vector2f uniformSphereToSquare(Vector3f v) {
	const Vector3f vn = v.normalized();
...
KRR_CALLABLE float squareToUniformSpherePdf() { return M_INV_4PI; }
KRR_CALLABLE float squareToUniformSpherePdfInv() { return M_4PI; }


KRR_CALLABLE Vector3f squareToUniformHemisphere(Vector2f u) {
	float z = u[0];
	float r = safe_sqrt(1 - z * z);
	float phi = M_2PI * u[1];
	return Vector3f(r * cos(phi), r * sin(phi), z);
...
KRR_CALLABLE Vector2f uniformHemisphereToSquare(Vector3f v) {
	const Vector3f vn = v.normalized();
	float phi = atan2(vn[1], vn[0]) * M_INV_2PI;
	return Vector2f(
		vn[2],
...
KRR_CALLABLE float squareToUniformHemispherePdf() { return M_INV_2PI; }
KRR_CALLABLE float squareToUniformHemispherePdfInv() { return M_2PI; }
