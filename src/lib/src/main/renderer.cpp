...
void RenderApp::loadConfig(const json config) {
	...
	if (config.contains("environment")) {
		...
		if (config["environment"].is_string()){
			string env = config["environment"].get<string>();
			scene->addInfiniteLight(InfiniteLight(env));
		}else{
			if (!config["environment"].contains("filename"))
				Log(Fatal, "No filename specified for environment light!");
			string env = config["environment"]["filename"].get<string>();
			float scale = config["environment"].value("scale", 1.f);
			scene->addInfiniteLight(InfiniteLight(env, scale));
		}
